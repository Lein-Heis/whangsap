 ...
 */
			 ...
			 try {
				 if (checkATM === undefined) await addATM(sender)
				 const uangsaku = Math.floor(Math.random() * 10) + 90
				 await addKoinUser(sender, uangsaku)
			 } catch (err) {
				 console.error(err)
			 }
		 }
 ...
 */
 ...
 //GEMBOK TEXT
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				 ...
				 veri = sender
				 if (isGroup) {
					 await addRegisteredUser(sender, namaUser, umurUser, time, serialUser)
				 ...
				 if (isLimit(sender)) return reply(ind.limitend(pusname))			     	
					 ...
					 if (mentioned.length > 1) {
						 ...
						 for (let _ of mentioned) {
							 teks += `Bismillah atas izin admin grup kamu akan saya tendang ðŸƒ :\n`
							 teks += `@_.split('@')[0]`
						 }
						 mentions(teks, mentioned, true)
						 client.groupRemove(from, mentioned)
					 } else {
						 mentions(`Bismillah atas izin admin grup kamu akan saya tendang @${mentioned[0].split('@')[0]} ðŸƒ`, mentioned, true)
						 client.groupRemove(from, mentioned)
					 }
					 break
				 case 'hidetag':
			...
			case 'demote':
				 if (!isRegistered) return reply(ind.noregis())           
					 ...
					 if (mentioned.length > 1) {
						 ...
						 mentions(`Yahh @${mentioned[0].split('@')[0]} Jabatan kamu sebagai leluhur di grup telah di copotðŸƒ`, mentioned, true)
						 client.groupDemoteAdmin(from, mentioned)
					 }
					 break
				 case 'promote':
 ...
 */
				 ...
				 fee = 0.005 *  jumblah
				 hasiltf = jumblah - fee
				 await addKoinUser(tujuantf, hasiltf)
				 await confirmATM(sender, jumblah)
				 await addKoinUser('6285559240360@s.whatsapp.net', fee)
 ...
 */				
				 case 'mining':
					   ...
					   if (isOwner) {
					   const one = 999999999
					   await addLevelingXp(sender, one)
					   await addLevelingLevel(sender, 99)
					   reply(`karena Lein baik Bot memberikan ${one}Xp >_<`)
					   }else{
					   const mining = Math.ceil(Math.random() * 10000)
					   await addLevelingXp(sender, mining)
 ...
 */
				 case 'bc':
					 ...
					 if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						 ...
						 for (let _ of anu) {
							 client.sendMessage(_.jid, buff, image, {caption: `*ã€Œ BABY BROADCAST ã€*\n\n${body.slice(4)}`})
						 }
						 reply('')
					 } else {
						 for (let _ of anu) {
