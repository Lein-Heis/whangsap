...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Lein\n' 
            + 'ORG: Owner Bot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=6287811186838:+62 878-1118-6838\n' 
            + 'END:VCARD'
prefix = '.'
blocked = []   
limitawal = 10
memberlimit = 9
cr = '*WHATSAPP THIS IS ALREADY VERIFIED*'

/******** OWNER NUMBER**********/
const devNumber = ["6289654360447@s.whatsapp.net"]
const ownerNumber = ["6287811186838@s.whatsapp.net"]
const pacarNumber = ["6281649050753@s.whatsapp.net"] 
/************************************/
 ...
 */
const _leveling = JSON.parse(await db.getDataLeveling())
const antilink = JSON.parse(await db.getDataAntilink())
const _level = JSON.parse(await db.getDataLevels())
const _registered = JSON.parse(await db.getDataRegistered())
const welkom = JSON.parse(await db.getDataWelkom())
const samih = JSON.parse(await db.getDataSamih())
const event = JSON.parse(await db.getDataEvent())
const _limit = JSON.parse(await db.getDataLimits())
const uang = JSON.parse(await db.getDataUang())
const ban = JSON.parse(await db.getDataBanned())
const prem = JSON.parse(await db.getDataPrem())
const wakil = JSON.parse(await db.getDataWakil())
 ...
 const getLevelingXp = (sender) => {
        ...
        const bayarLimit = async (sender, amount) => {
            ...
            if (position !== false) {
                ...
                await db.updateDataLimits(JSON.stringify(_limit))
            }
        }
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `@${num.split('@')[0]}\n*Yo Mamang Pake Bot Jgn Spam Ya*`
				...
				teks = `*Ga Usah Masuk Lagi Bero* @${num.split('@')[0]}\n`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
        ...
        if (levelRole <= 2) {
         ...
         if (isGroup) {
            ...
            if (checkAfkUser(sender) && !isCmd) {
                ...
                await db.updateDataAfk(JSON.stringify(_afk))
		...
		setTimeout( () => {
			client.groupRemove(from, [kicc]).catch((e)=>{reply(`*ERR:* ${e}`)})
                    ...
                    return reply( "Tolong Jaga Kata2nya")
                }
            }
        }
		...
		case 'tebakgambar':
				...
				/*case 'caklontong':			
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://docs-api-zahirr.herokuapp.com/api/kuis/caklontong?apikey=zahirgans`, {method: 'get'})
					caklontong = await getBuffer (anu.result.soal)
					setTimeout( () => {
		...
		case 'transfer':
				...
				if (!q.includes('|')) return  reply(ind.wrongf())
                		...
                		addKoinUser('6287811186838@s.whatsapp.net', fee)
              ...
              case 'bot':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				tels = body.slice(5)
				let ainezcb = fs.readFileSync('./mp3/WhatsApp-Audio-2021-03-02-at-20.52.32-_2_.opus')
				/*let ainescb = fs.readFileSync('./mp3/WhatsApp-Audio-2021-03-10-at-20.33.57.opus')*/
				client.sendMessage(from, 'Iya apa kak?\nJangan lupa daftar iya kak >,<"',MessageType.text, { quoted: mek} )
				client.sendMessage(from, ainezcb, MessageType.audio, { quoted: mek, ptt: true })
				/*client.sendMessage(from, ainescb, MessageType.audio, { quoted: mek, ptt: true })*/
				await limitAdd(sender)
				break
              case 'terima':
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				tels = body.slice(8)
				let ainezcbh = fs.readFileSync('./mp3/WhatsApp-Ptt-2021-03-10-at-20.35.07.opus')
				client.sendMessage(from, ainezcbh, MessageType.audio, { quoted: mek, ptt: true })
				await limitAdd(sender)
				break
              ...
              case 'sayang':
				...
				let ainezcbhh = fs.readFileSync('./mp3/WhatsApp-Audio-2021-03-10-at-20.46.49.opus')
				client.sendMessage(from, ainezcbhh, MessageType.audio, { quoted: mek, ptt: true })
				await limitAdd(sender)
				break
               /*case 'bot':
              ...
              case 'asalamualaikum':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				tels = body.slice(16)
				let ainezzs = fs.readFileSync('./mp3/WhatsApp-Audio-2021-03-02-at-20.46.15.opus')
				client.sendMessage(from, 'Waalaikumusalam Warahmatullahi Wabarakatuh ',MessageType.text, { quoted: mek} )
				client.sendMessage(from, ainezzs, MessageType.audio, { quoted: mek, ptt: true })
				await limitAdd(sender)
				break
              ...
              case 'hello':
				...
				client.sendMessage(from, aineesii, audio, {mimetype: 'audio/mp4', filename: `hellolittlegirl.mp3`, quoted: mek})
				await limitAdd(sender)
				break
              case 'huaaaaaaa':
              case 'huaaaaaa':
              ...
              case 'hua':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (!isPremium) return reply('Maaf kamu bukan user premium!')
				let huuuaaa = fs.readFileSync('./mp3/WhatsApp-Audio-2021-03-03-at-00.32.11.opus')
				client.sendMessage(from, huuuaaa, MessageType.audio, { quoted: mek, ptt: true })
				await limitAdd(sender)
				break
                /*case 'userbotlist':
				client.updatePresence(from, Presence.composing) 
				if (!isRegistered) return reply(ind.noregis())    
				teks = 'This is list of register bot number :\n'
				for (let regis of _registered) {
					teks += `~> wa.me/${regis.split('@')[0]}\n`
					}
					teks += `Total : ${_registered.length}`
				client.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": _registered}})
				break*/
                case 'admin':
				if (!isOwner) return reply(ind.ownerb())
				admm = body.slice(7)
				wakil.push(`${admm}@s.whatsapp.net`)
				await db.updateDataWakil(JSON.stringify(wakil))
				...
				await db.updateDataPrem(JSON.stringify(prem))
		...
		case 'unpremium':
				...
				await db.updateDataBanned(JSON.stringify(ban))
		...
		case 'giftlimit': 
				...
				const nomerr = args[0].replace('@','')
                		...
                		if (!nomerr) return reply(`Maaf format salah\nmasukan parameter yang benar\ncontoh : ${prefix}giftlimit @6287811186838 20`)
		...
		case 'text3d':
				...
				client.sendMessage(from, corn, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
                case 'glitch':
				if (!isRegistered) return reply(ind.noregis())
				...
				if (args.length < 1) return reply(ind.wrongf())
				reply(ind.wait())
				qissya = await getBuffer(`https://api.zeks.xyz/api/gtext?text1=${tels3}&text2=${tels4}&apikey=apivinz`)
				client.sendMessage(from, qissya, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
                case 'wolflogo':
				if (!isRegistered) return reply(ind.noregis())
				...
				zeks2 = gh.split("|")[1];
				reply(ind.wait())
				qiss = await getBuffer(`https://api.zeks.xyz/api/wolflogo?apikey=apivinz&text1=${zeks1}&text2=${zeks2}`)
				client.sendMessage(from, qiss, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
                case 'phlogo':
				if (!isRegistered) return reply(ind.noregis())
		...
		case 'nulis':
				...
				client.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi.. tulisan koboy baguskan? 🤭', quoted: mek})
				await limitAdd(sender)
				break
		/*case 'findgooglepict':
				if (!isRegistered) return reply(ind.noregis())
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				if (args.length < 1) return reply(ind.wrongf())
				toodis = body.slice(16)
				reply(ind.wait())
				toodis = await getBuffer(`http://lolhuman.herokuapp.com/api/gimage?apikey=WEMPYGANSS&query=${toodis}`)
				client.sendMessage(from, toodis, image, {caption: 'Nih kak pencarian koboy', quoted: mek})
				await limitAdd(sender)
				break
		case 'nulis3':
				if (!isRegistered) return reply(ind.noregis())
				...
				toods = body.slice(7)
				reply(ind.wait())
				toods = await getBuffer(`https://lolhuman.herokuapp.com/api/nulis?apikey=WEMPYGANSS&text=${toods}`)
				client.sendMessage(from, toods, image, {caption: 'Nih kak udah jadi.. tulisan koboy baguskan? 🤭', quoted: mek})
				await limitAdd(sender)
				break*/
		case 'nulis2':
				if (!isRegistered) return reply(ind.noregis())
				...
				aizu = body.slice(8)
				reply(ind.wait())
				data = await fetchJson(`https://docs-api-zahirr.herokuapp.com/api/nulis?text=${aizu}&apikey=zahirgans`)
				buffer = await getBuffer(data.result.url)
				client.sendMessage(from, buffer, image, {caption: 'Nih kak udah jadi.. tulisan koboy baguskan? 🤭', quoted: mek})
				await limitAdd(sender)
				break
		/*case 'nulis2':
		...
		/*case 'hartatata2':
				...
				pt = body.slice(12)
				reply(ind.wait())
				bh = await getBuffer(`https://api.vhtear.com/hartatahta?text=${pt}&apikey=c1d162b46e634f389efa1ac715f03d03`)
				client.sendMessage(from, bh, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break*/
		/*case 'thunder':
				...
				cpp = body.slice(9)
				reply(ind.wait())
				qibu = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome/thunder?apikey=WEMPYGANSS&text=${cpp}`)
				client.sendMessage(from, qibu, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break*/
		case 'imgmaker':
				if (!isRegistered) return reply(ind.noregis())
		...
		case 'shota':
				...
				gatauda = body.slice(8)
				reply(ind.wait())
				anu = await getBuffer(`http://lolhuman.herokuapp.com/api/meme/memeindo?apikey=WEMPYGANSS`, {method: 'get'})
		...
		/*case 'ytmp3':
				...
				if (args.length < 1) return reply('Urlnya mana kak?')
				if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(ind.stikga())
				anu = await fetchJson(`https://lolhuman.herokuapp.com/api/ytaudio?apikey=WEMPYGANSS&url=${args[0]}`, {method: 'get'})
				if (anu.error) return reply(anu.error)
				teks = `❏  ${anu.result.title}\n❏ *Ukuran* : ${anu.result.link.size}\n\n❏ *Tunggu Bentar Ya Kak, Audionya Lagi Di Kirim...*`
				thumb = await getBuffer(anu.result.thumbnail)
				client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
		...
		/*case 'fbmp4':
				...
				if (args.length < 1) return reply('Urlnya mana kak?')
				play = body.slice(5)
				anu = await fetchJson(`https://mnazria.herokuapp.com/api/fbdownloadervideo?url=${play}`, {method: 'get'})
		...
		/*case 'tiktokdl': 
				...
				if (!isUrl(args[0]) && !args[0].includes('tiktok.com')) return reply(ind.stikga())
				ige = body.slice(10)
		...
		/*case 'play':   
				...
				reply(ind.wait())
				play = body.slice(5)
				anu = await fetchJson(`https://lolhuman.herokuapp.com/api/ytplay?apikey=WEMPYGANSS&query=${play}`)
				if (anu.error) return reply(anu.error)
				infomp3 = `*Lagu Ditemukan!!!*\nJudul : ${anu.result.info.title}\nSource : ${anu.result.source}\nUkuran : ${anu.result.audio.size}\n\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM YA SAYANG*`
		...
		case 'gliitch':
				...
				reply(ind.wait())
				data = await getBuffer(`https://docs-api-zahirr.herokuapp.com/api/textmaker?text=${aruga1}&text2=${aruga2}&theme=glitch&apikey=zahirgans`)
				buffer = await getBuffer(data.result.url)
		...
		case 'blood':
				...
				aruga = body.slice(7)
				reply(ind.wait())
				aruga = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome/bloodfrosted?apikey=WEMPYGANSS&text=${aruga}`)
				client.sendMessage(from, aruga, image, {caption: 'Nih kak', quoted: mek})
				await limitAdd(sender)
				break
		case 'firework':
		...
		/*case 'animequotes':
					...
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					data = await fetchJson(`http://lolhuman.herokuapp.com/api/random/quotesnime?apikey=WEMPYGANSS`)
					bas = `Quotes : _${data.result.quote}_\nKarakter : _${data.result.character}_\nAnime : _${data.result.anime}_`
					reply(bas)
					await limitAdd(sender)
					break*/
                case 'twichquotes':
                ...
                /*case 'katabijak':
					...
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/katabijak?apikey=WEMPYGANSS`, {method: 'get'})
					reply(anu.result)
					await limitAdd(sender)
					break*/
		case 'quotes':
					...
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					data = await fetchJson(`https://videfikri.com/api/randomquotes/`)
					cop = `_${data.result.quotes}_\n\n_${data.result.author}_`
					reply(cop)
					await limitAdd(sender)
					break
		case 'simii':
					if (!isRegistered) return reply(ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					data = await fetchJson(`https://videfikri.com/api/simsimi/?teks=${body.slice(7)}`)
					reply(data.result.jawaban)
					break
		/*case 'cerpen':
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					data = await fetchJson(`http://lolhuman.herokuapp.com/api/cerpen?apikey=WEMPYGANSS`)
					cerp = `Judul : _${data.result.title}_\nPembuat : _${data.result.creator}_\n\n${data.result.cerpen}`
					reply(cerp)
					await limitAdd(sender)
					break*/
		case 'cerpen':
		...
		/*case 'kata':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if (args.length < 1) return reply('kasih teks lah^_^!!!')
                                        aineizi = body.slice(5)
					data = await fetchJson(`http://lolhuman.herokuapp.com/api/quotes/dilan?apikey=WEMPYGANSS`)
					copppi = `_${data.result}_`
					reply(copppi)
					await limitAdd(sender)
					break*/
		case 'ccgenerator':
		...
		case 'quotesmuslim':
					...
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					data = await fetchJson(`https://docs-api-zahirr.herokuapp.com/api/randomquote/muslim?apikey=zahirgans`)
		...
		case 'findnabi':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					break
                case 'translate':
                                        aineizi = body.slice(10)
                                        lang = aineizi.split("|")[0];
                                        teksnya = aineizi.split("|")[1];
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        if (args.length < 1) return reply(`Kode bahasanya mana kak?\nContoh: ${prefix}translate en|Hai, aku Aine`)
                                        if (args.length < 2) return reply(`Teksnya mana kak?\nContoh: ${prefix}translate en| Hai, aku Aine`)
                                        anu = await fetchJson(`https://api-translate.azharimm.tk/translate?engine=google&text=${teksnya}&to=${lang}`, {method: 'get'})
                                        arteks = `◪ *TRANSLATE* \n  │\n  ├❑ Text : ${anu.data.origin} \n  └❑ Translate : ${anu.data.result}`
                                        reply(arteks)
                                        await limitAdd(sender)
                                        break
		case 'covid':
		...
		case 'niatshalat':
					...
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					data = await fetchJson(`https://docs-api-zahirr.herokuapp.com/api/muslim/niatshalat?apikey=zahirgans`, {method: 'get'})
					teks = '=================\n'
					for (let i of data.result) {
		...
		case 'asmaulhusna':
					...
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					data = await fetchJson(`https://docs-api-zahirr.herokuapp.com/api/muslim/asmaulhusna?apikey=zahirgans`, {method: 'get'})
					teks = '=================\n'
		...
		case 'alay':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if (args.length < 1) return reply('kasih teks lah^_^!!!')
					data = await fetchJson(`https://lolhuman.herokuapp.com/api/alay?apikey=WEMPYGANSS&text=${body.slice(6)}`)
					reply(data.result)
				        await limitAdd(sender)
					break
		case 'namaninja':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if (args.length < 1) return reply('kasih teks lah^_^!!!')
					data = await fetchJson(`https://docs-jojo.herokuapp.com/api/ninja_name?name=${body.slice(11)}`)
					reply(data.result)
				        await limitAdd(sender)
					break
		case 'spamcall':
		...
		/*case 'pantun':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					gatauda = body.slice(8)					
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/pantun?apikey=WEMPYGANSS`, {method: 'get'})
					reply(anu.result)
					await limitAdd(sender)
					break*/
		case 'resepmasakan':
		...
		case 'grouplist':
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group KOBOY BOT :\n\n\`\`\``
					no = 0
		...
		case 'daftar':
                			...
                			if (umurUser < 10) return reply(`your age is too young minimum 12 years`)
                					try {
		...
		case 'mining':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					const one = Math.ceil(Math.random() * 9999)
					addLevelingXp(sender, one)
					await reply(`Kamu bagian dari prabayar, aku akan berikan sebanyak *${one}Xp* untuk anda`)
                 					     }else{
					const mining = Math.ceil(Math.random() * 999)
		...
		case 'infoadmin':
					me = client.user
					uptime = process.uptime()
					teks = `*INFO ADMIN*\n*Admin bot* : Akagami\n*No akagami* : wa.me/6287811186838\n*Ig akagami* : www.instagram.com/arzyle.fx\n━━━━━━━━━━━━━━━━━━━━\n*INFO BOT*\n*Nama bot* : ${me.name}\n*Nomor bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total block contact* : ${blocked.length}\n*The bot is active on* : ${kyun(uptime)}\n*Fans page FB* : https://www.facebook.com/ainneboot\n*Ketik* : ${prefix}lpr _Untuk melaporkan admin bot melalui bot_\n*Ketik* : ${prefix}owner untuk menghubungi admin bot kami.`
					buffer = await getBuffer(`https://i.ibb.co/4jknX6w/pp.jpg`)
					client.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
		case 'snk':
		...
		case 'lpr':
					...
					client.sendMessage('6287811186838@s.whatsapp.net', options, text, {quoted: mek})
		...
		case 'request':
					...
					client.sendMessage('6287811186838@s.whatsapp.net', options, text, {quoted: mek})
					reply('REQUEST ANDA TELAH SAMPAI KE OWNER DAN ADMIN KOBOYBOT\nRequests palsu atau main² tidak akan ditanggapi.')
					break
		case 'requestlibdhdjdmit':
					...
					client.sendMessage('6282237617224@s.whatsapp.net', options, text, {quoted: mek})
					client.sendMessage('6282148618789@s.whatsapp.net', options, text, {quoted: mek})
					client.sendMessage('628994622890@s.whatsapp.net', options, text, {quoted: mek})
					client.sendMessage('6289654360447@s.whatsapp.net', options, text, {quoted: mek})
					client.sendMessage('6283133940686@s.whatsapp.net', options, text, {quoted: mek})
					reply('REQUEST LiMIT ANDA TELAH SAMPAI KE OWNER DAN ADMIN AINEBOT\nRequests palsu atau main² tidak akan ditanggapi.')
					break
		case 'smsfredbnde':
					if (!isPremium) return reply('Maaf kamu bukan user premium!')
		...
		case 'blocklist': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                                ...
                                var ainnne = mek.participant
                                const tagzz = {
                                                text: `@${ainnne.split("@s.whatsapp.net")[0]}  *Sedang  afk ${tels}  jangan  ganggu  ya*`,
                                                contextInfo: { mentionedJid: [ainnne] }
                                        }
		...
		case 'quotesmotivasi':
                ...
                case 'fotocewek':
					...
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
	                                const cangti = ['https://i.ibb.co/XJCg3wz/cewek7.jpg','https://i.ibb.co/1sf4nRy/cewek6.jpg','https://i.ibb.co/hmcBxNr/cewek5.jpg','https://i.ibb.co/0JsLWkX/foto-cewek2-cantik-lucu-berhijab-untuk-quotes-wallpaper-foto-cewek2-cantik-lucu-berhijab-paimin-gamb.jpg','https://i.ibb.co/bsCmwWn/cewek1.jpg','https://i.ibb.co/8YqW10c/cewek2.jpg','https://i.ibb.co/FK9tLYL/Cewek-Cantik-Biodatalengkap-1-10.jpg']
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
	                                const cowok = ['']
                                        let cowo = cowok[Math.floor(Math.random() * cowok.length)]
                                        client.sendMessage(from, cowo, image, {quote: mek})
					await limitAdd(sender)
                                        break*/
                case 'kata':
					if (!isRegistered) return reply(ind.noregis())
                ...
                /*case 'menherachan':
					...
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
	                                const menhera = ['https:\/\/i.pinimg.com\/originals\/83\/48\/aa\/8348aaf8e92c70402bd99b847205d4c7.webp','https:\/\/i.pinimg.com\/originals\/ed\/91\/0f\/ed910f129621354dfed69a6fc016c4d8.jpg','https:\/\/i.pinimg.com\/originals\/70\/60\/85\/7060854734e1c4c4d2651cface40bcc8.png','https:\/\/i.pinimg.com\/originals\/f1\/34\/93\/f13493d78cde01c1325d2b253557523e.jpg','https:\/\/i.pinimg.com\/originals\/6e\/ae\/e0\/6eaee08423515dce0935fb556110a41b.jpg','https:\/\/i.pinimg.com\/originals\/85\/b8\/2a\/85b82ada63be6d67738197457e2ff6cc.jpg','https:\/\/i.pinimg.com\/originals\/ca\/da\/46\/cada46524c840f8e9d1310b4b41658f3.jpg','https:\/\/i.pinimg.com\/originals\/f6\/e1\/90\/f6e190ba3a2c8646e634908ba2e1443d.jpg','https:\/\/i.pinimg.com\/originals\/9c\/7e\/9d\/9c7e9d1adbacef506b271ea7c85e2235.png','https:\/\/i.pinimg.com\/originals\/e3\/65\/b0\/e365b0c5e13c17651aa53a14d6dece38.jpg','https:\/\/i.pinimg.com\/originals\/a9\/5a\/0d\/a95a0d424f8d7a9ae236a28f2114a09e.png','https:\/\/i.pinimg.com\/originals\/db\/52\/db\/db52db21d4fa0a7e320c5bfe3672c469.jpg','https:\/\/i.pinimg.com\/originals\/71\/68\/70\/716870dde49840093900257d7b0603c9.jpg','https:\/\/i.pinimg.com\/originals\/66\/05\/0f\/66050fd223a6678e49e9c1cd1133f9c2.jpg','https:\/\/i.pinimg.com\/originals\/9d\/09\/6c\/9d096c3a6a51413edee5385ca50dd0a2.jpg','https:\/\/i.pinimg.com\/originals\/60\/aa\/b0\/60aab08e4327ebb44682442cf8efeef1.jpg','https:\/\/i.pinimg.com\/originals\/89\/a0\/0b\/89a00ba60c3f5515dc900785b2b97649.png','https:\/\/i.pinimg.com\/originals\/83\/4f\/3c\/834f3c5b352e6231163e72a514dfd881.jpg','https:\/\/i.pinimg.com\/originals\/b0\/13\/13\/b0131382ef5e2ca5f51e519b1dd905eb.jpg','https:\/\/i.pinimg.com\/originals\/4a\/34\/c0\/4a34c0eb4a53e29f503fa31d1dcb5020.jpg','https:\/\/i.pinimg.com\/originals\/dc\/fc\/f1\/dcfcf13d984550e60b6e299fe511f73e.png','https:\/\/i.pinimg.com\/originals\/19\/1e\/25\/191e256585fd98a09391fb5c4332459c.jpg','https:\/\/i.pinimg.com\/originals\/a6\/ee\/58\/a6ee587d51d7b99278d88d21c2aef822.jpg','https:\/\/i.pinimg.com\/originals\/9f\/d4\/7c\/9fd47c18a17261587e37a47fa272d552.jpg','https:\/\/i.pinimg.com\/originals\/6f\/9e\/58\/6f9e58711160f96c8490fcda4536e8bb.jpg','https:\/\/i.pinimg.com\/originals\/e9\/0e\/96\/e90e96a2258e602753a4f30c1677f14e.png','https:\/\/i.pinimg.com\/originals\/df\/fc\/1c\/dffc1c1a463726923f78de0f17447e83.jpg','https:\/\/i.pinimg.com\/originals\/c8\/72\/7b\/c8727bb9d31137634b87e6e91c6ed8bf.png','https:\/\/i.pinimg.com\/originals\/19\/cd\/33\/19cd33747f4512bd9d037ef49a3ac699.jpg','https:\/\/i.pinimg.com\/originals\/82\/63\/f7\/8263f7bc6dbb189edee9c539a46f3dfc.jpg','https:\/\/i.pinimg.com\/originals\/27\/ef\/5e\/27ef5eb70efdc50bacc8a6ce78e92388.jpg','https:\/\/i.pinimg.com\/originals\/c7\/fd\/85\/c7fd857c6c18f32f806f1bac7ad69361.png','https:\/\/i.pinimg.com\/originals\/a8\/5d\/7c\/a85d7c62967338183e75c46afe1255f0.jpg','https:\/\/i.pinimg.com\/originals\/05\/e3\/a8\/05e3a8c7ddacdf9a4d8332ae475652e1.jpg','https:\/\/i.pinimg.com\/originals\/c2\/78\/8f\/c2788f5e279f85ad35f09828a97cfb34.jpg','https:\/\/i.pinimg.com\/originals\/31\/a3\/f6\/31a3f6904f7f49fa0f90b30e0e9052b9.jpg','https:\/\/i.pinimg.com\/originals\/ea\/f6\/88\/eaf688b3d52a66ecab38d159182cf5bc.jpg','https:\/\/i.pinimg.com\/originals\/58\/b0\/c8\/58b0c80f0bc01895a84f8f8765cb8987.png','https:\/\/i.pinimg.com\/originals\/17\/11\/44\/171144a83598330fd930948c15304943.jpg','https:\/\/i.pinimg.com\/originals\/05\/1c\/38\/051c3860cd7105f1814119df47f28b11.jpg','https:\/\/i.pinimg.com\/originals\/87\/74\/02\/8774023c28390cfe410227539d38c863.jpg','https:\/\/i.pinimg.com\/originals\/cc\/eb\/34\/cceb34e7424948f6360f0e77b953a130.png','https:\/\/i.pinimg.com\/originals\/5d\/d0\/4d\/5dd04dd06efb686f003c07ba855ada92.jpg','https:\/\/i.pinimg.com\/originals\/8d\/f3\/d1\/8df3d191dca29e7c692543de2993c93d.png','https:\/\/i.pinimg.com\/originals\/0b\/ae\/25\/0bae2596080bbe0aea8cfb5a479443ae.jpg','https:\/\/i.pinimg.com\/originals\/c5\/c2\/d0\/c5c2d0a229ed16c22f48263b7e118cbc.png','https:\/\/i.pinimg.com\/originals\/d0\/e4\/0a\/d0e40ae38b08c9e9508c26b63e5990b8.png','https:\/\/i.pinimg.com\/originals\/33\/9b\/1d\/339b1df9d30fc2bc4436c326686be79f.jpg','https:\/\/i.pinimg.com\/originals\/a7\/a3\/d4\/a7a3d401df9e0154d7ad7c52fd9b3fd9.jpg','https:\/\/i.pinimg.com\/originals\/18\/13\/f0\/1813f065c4c6b2471e9b6bafc3eb0ace.jpg']
                                        let chan = menhera[Math.floor(Math.random() * menhera.length)]
                                        client.sendMessage(from, chan, image, {quote: mek})
					await limitAdd(sender)
                                        break*/
                /*case 'kucing':
					if (!isRegistered) return reply(ind.noregis())
		...
		/*case 'fototiktok':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					gatauda = body.slice(12)
					anu = await getBuffer(`http://lolhuman.herokuapp.com/api/pptiktok/${gatauda}?&apikey=WEMPYGANSS` , {method: 'get'})
		...
		case 'tts':
				...
				if (args.length < 1) return client.sendMessage(from, 'Diperlukan kode bahasa kak!!', text, {quoted: mek})
					...
					if (args.length < 2) return client.sendMessage(from, 'Mana teks yang mau di jadiin suara? suara lu kah?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('Textnya kebanyakan bero!! 😤')
		...
		case 'setprefixdev':
					if (args.length < 1) return
					if (!isDev) return reply('Fitur hanya untuk developer!')
					prefix = args[0]
					reply(`*Prefix berhasil di ubah menjadi* : ${prefix}`)
					break
		case 'setlimit':
		...
		case 'tiktokstalk':
				...
				break
		case 'kick':
					client.sendMessage(from, 'Maaf fitur kick tidak tersedia!\nKarena rentan benned oleh pihak whatsapp.',MessageType.text, { quoted: mek} )
					break
		case 'add':
					client.sendMessage(from, 'Maaf fitur add tidak tersedia!\nKarena rentan benned oleh pihak whatsapp.',MessageType.text, { quoted: mek} )
					break
		case 'promote':
					client.sendMessage(from, 'Maaf fitur promote tidak tersedia!\nKarena rentan benned oleh pihak whatsapp.',MessageType.text, { quoted: mek} )
					break
		case 'demote':
					client.sendMessage(from, 'Maaf fitur demote tidak tersedia!\nKarena rentan benned oleh pihak whatsapp.',MessageType.text, { quoted: mek} )
					break
		case 'gckoboybot':
					client.sendMessage(from, 'https://chat.whatsapp.com/Ee1EBuqZyTs9NqXMUSg43C',MessageType.text, { quoted: mek} )
					break
		/*case 'Hi':
					client.sendMessage(from, 'Apa sayangku? 🥰',MessageType.text, { quoted: mek} )
					client.sendMessage(from, 'Kamu lagi apa sayang?',MessageType.text, { quoted: mek} )
					break*/
		case 'duduk':
					tels = body.slice(7)
					client.sendMessage(from, 'Ohh.. duduk..',MessageType.text, { quoted: mek} )
					client.sendMessage(from, 'Duduk sambil apa?',MessageType.text, { quoted: mek} )
					break
		case 'gelud':
					tels = body.slice(7)
					client.sendMessage(from, 'Kuylah, dapat donasi aku..',MessageType.text, { quoted: mek} )
					break
		case 'sms':
		...
		case 'test':
					...
					break
		case 'ping':
					client.sendMessage(from, 'ping',MessageType.text, { quoted: mek} )
					break
		case 'kontol':
  					if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)
					client.sendMessage(from, 'Gunakan bahasa yang benar\nAnda akan kami banned!\nHubungi kami : wa.me/6287811186838',MessageType.text, { quoted: mek} )
					await limitAdd(sender)
					break
		case 'memek':
  					if (isLimit(sender)) return reply(ind.limitend(pusname))
		...
		case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST KOBOYBOT 」*\n\nDari : Owner\nPengirim : wa.me/${(sender.split('@')[0])}\nPesan : ${body.slice(4)}`)
						}
						reply('*Suksess broadcast* ')
					}
					break
		...
		case 'toimg':
				...
				if (!isQuotedSticker) return reply('*Reply/Tag sticker!*')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '*DAH JADI BERO* '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
		...
		case 'addbadword':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply( `Kirim perintah ${prefix}addbadword [kata kasar]. contoh ${prefix}addbadword Ngent0d`)
					const bw = body.slice(12)
					bad.push(bw)
					await db.updateDataBadword(JSON.stringify(badword))
		...
		case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						samih.push(from)
						await db.updateDataSamih(JSON.stringify(samih))
		...
		case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						nsfw.push(from)
						await db.updateDataNsfw(JSON.stringify(_leveling))
		...
		case 'leveling':
					...
					_leveling.push(from)
					await db.updateDataLeveling(JSON.stringify(_leveling))
					reply(ind.lvlon())
		...
		case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						welkom.push(from)
						await db.updateDataWelkom(JSON.stringify(welkom))
                 ...
                 case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						antilink.push(from)
						await db.updateDataAntilink(JSON.stringify(antilink))
                 ...
                 case 'event':
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply('*Only Owner Us!*')
					if (args.length < 1) return reply('Mengaktifkan tekan 1, Menonaktif tekan 0')
					if (Number(args[0]) === 1) {
						if (isEventon) return reply('*Fitur event sudah aktif sebelum nya*')
						event.push(from)
						await db.updateDataEvent(JSON.stringify(event))
