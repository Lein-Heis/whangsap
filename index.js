...
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN ASU!?
            ...
            + 'FN:Ramdan Ganz\n' // NAMA OWNER BOT
            + 'ORG:Creator REAMVBOT;\n' // KASIH CREDITS GUA FADHIL GRAPHY SU!!!
            + 'TEL;type=CELL;type=VOICE;waid=6289609925483:+62 896-0992-5483\n' // GANTI NOMOR LU
            + 'END:VCARD'
prefix = '/'
blocked = []
...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6289609925483@s.whatsapp.net"] // GANTI NOMOR LU
			const adminbotnumber = ["6289609925483@s.whatsapp.net"] // GANTI NOMOR LU
			const frendsowner = ["6282119816316@s.whatsapp.net"] // GAUSAH DIGANTI!!!
			...
			case 'owner':
                ...
                case 'ninjalogo':
                      ...
                      gl2 = gh.split("|")[1];
                      reply(mess.wait)
                      anu = await fetchJson(`https://tobz-api.herokuapp.com/api/textpro?theme=ninjalogo&text1=${gl1}&text2=${gl2}&apikey=${tobzApi}`, {method: 'get'})
                ...
                case 'wolflogo':
                      ...
                      gl2 = gh.split("|")[1];
                      reply(mess.wait)
                      anu = await fetchJson(`https://tobz-api.herokuapp.com/api/textpro?theme=wolflogo1&text1=${gl1}&text2=${gl2}&apikey=${tobzApi}`, {method: 'get'})
                      buff = await getBuffer(anu.result)
                      client.sendMessage(from, buff, image, {quoted: mek})
                      break
                case 'lionlogo':
                      ...
                      gl2 = gh.split("|")[1];
                      reply(mess.wait)
                      anu = await fetchJson(`https://tobz-api.herokuapp.com/api/textpro?theme=lionlogo&text1=${gl1}&text2=${gl2}&apikey=${tobzApi}`, {method: 'get'})
                      buff = await getBuffer(anu.result)
                      client.sendMessage(from, buff, image, {quoted: mek})
                      break
				case 'leave': 
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
                ...
                case 'report':
                    ...
                    client.sendMessage('6289609925483@s.whatsapp.net', options, text, {quoted: mek})
