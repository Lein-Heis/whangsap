...
ramlan = body.slice(7)
reply('[❗] SABAR BANG GA SABAR GELUD KYTA')
buff = await getBuffer(`https://api.vhtear.com/write?text=${ramlan}&apikey=${vhtear}`)
client.sendMessage(from, buff, image, {quoted: mek, caption: 'Nulis sendiri gblok'})
await limitAdd(sender)
break					
case 'quotes':
if (isBanned) return reply(ind.baned())
...
break
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return client.sendMessage(from, 'Kode bahasanya mana Bang? | contoh : ${prefix}tts id ah yamate kudasai', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Teksnya mana Bang? | contoh : ${prefix}tts id ah yamate kudasai', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('Teks nya terlalu panjang bang')
                ...
                var gh = body.slice(12)
					...
					client.sendMessage(from, buffer, image, {caption: 'Nih Bodoh Kau', quoted: mek})
					await limitAdd(sender)
					break
                   case 'stalkig':
                   if (isBanned) return reply(ind.baned())
                ...
                if (namaUser.length >= 30) return reply(`Namanya kepanjangan Bodoh :)`)
...
*/
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					const bisa =['Tentu Saja Bisa! Kamu Adalah Orang Paling Homky','Gak Bisa Ajg Aowkwowk','Gw G Tau, tanya ama bapakau','Ulangi Bodoj G Tau Gw']
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
					apakah = body.slice(1)
					const apa =['Iya Seneng Lu?','Tidak Yahaha','Bisa Jadi','Ulangi bro gak paham']
